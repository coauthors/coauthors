...
export const convert = async (message: string): Promise<MessageInfo> => {
  ...
  const coauthorsLineIndex = lines.findIndex((line) => line.startsWith(COAUTHORS_PREFIX))
  const coauthorsLine = lines.find((line) => line.trimStart().startsWith(COAUTHORS_PREFIX))

  const authors = coauthorsLine?.substring(COAUTHORS_PREFIX.length, coauthorsLine.length).split(',').map(parseAuthor)
...
const parseAuthor = (inputString: string): Author => {
  ...
  if (parenthesesIndex.open >= 0 && parenthesesIndex.close >= 0) {
    return {
      ...
      name: input.slice(parenthesesIndex.open + 1, parenthesesIndex.close).trim(),
    }
  }
